...
export default class Manager {
    ...
    async getGistSnippets() {
        return await this._getGistSnippets();
    }
        ...
        return axios
            ...
            .then(files => {
                return Promise.all(files.map(file =>
                    axios.get(`/gists/${file.id}`, options)
                        .then(response => {
                            const { data } = response;
            ...
            .then(files => {
                return files.map(file => {
                    try {
                        ...
                        atom.notifications
                            .addError(`Could not parse ${file.filename} : ${e.message}`);
                    }
                })
            });
    }
